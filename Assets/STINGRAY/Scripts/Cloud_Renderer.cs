...
public class Cloud_Renderer : ScriptableRendererFeature
{
    class CustomRenderPass : ScriptableRenderPass
    {
        ...
        public CustomRenderPass(Material mat, int downSampleAmount)
        {
			...
			tempRenderTarget2 = RTHandles.Alloc("_TemporaryColourTexture", name: "_TemporaryColourTexture");
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
			...
			cmd.GetTemporaryRT(downSample, cameraTargetDescriptor, FilterMode.Bilinear);
			cmd.GetTemporaryRT(downSample*3, originalDescriptor, FilterMode.Bilinear);
		}
		...
		public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            if (renderingData.cameraData.cameraType != CameraType.Reflection)
            {
				...
				int nameID = 0;
				string pattern = @"NameID\s(\d+)";
				Match match = Regex.Match(tempRenderTarget.nameID.ToString(), pattern);
				if (match.Success)
				{
					nameID = int.Parse(match.Groups[1].Value);
				}
					commandBuffer.GetTemporaryRT(nameID, downscaleCam);

				//commandBuffer.SetGlobalTexture("_CloudsTex", tempRenderTarget.Identifier());
				commandBuffer.SetGlobalTexture("_CloudsTex", tempRenderTarget.nameID);
