Shader "Ciconia Studio/CS_Ghost/CS_Advanced Ghost (Always Visible)"
{
	...
	SubShader
	{
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			float3 NormalmapXYZ536 = UnpackScaleNormal( tex2D( _BumpMap, uv0_BumpMap ), _BumpScale );
			...
			#endif
			float4 lerpResult559 = lerp( ( ( _Color * tex2D( _MainTex, uv0_MainTex ) ) * float4( ( ( max( dotResult540 , 0.0 ) * ( ase_lightAtten * ase_lightColor.rgb ) ) + indirectDiffuse550 ) , 0.0 ) ) , float4( indirectSpecular557 , 0.0 ) , staticSwitch556);
			float4 CustomLight560 = lerpResult559;
			c.rgb = CustomLight560.rgb;
			c.a = Opacity250;
			return c;
		}
