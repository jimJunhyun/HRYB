Shader "Ciconia Studio/CS_Ghost/CS_Advanced Ghost (XRay See Through)"
{
	...
	SubShader
	{
		...
		void outlineSurf( Input i, inout SurfaceOutput o )
		{
			...
			o.Alpha = ( fresnelNode536 * _XRayColor.a );
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			float3 NormalmapXYZ572 = UnpackScaleNormal( tex2D( _BumpMap, uv0_BumpMap ), _BumpScale );
			float3 ase_worldPos = i.worldPos;
			...
			#endif
			float4 lerpResult595 = lerp( ( ( _Color * tex2D( _MainTex, uv0_MainTex ) ) * float4( ( ( max( dotResult577 , 0.0 ) * ( ase_lightAtten * ase_lightColor.rgb ) ) + indirectDiffuse585 ) , 0.0 ) ) , float4( indirectSpecular594 , 0.0 ) , staticSwitch592);
			float4 CustomLight596 = lerpResult595;
			c.rgb = CustomLight596.rgb;
			c.a = Opacity250;
			return c;
		}
		...
		void surf( Input i , inout SurfaceOutputCustomLightingCustom o )
		{
			...
			float3 temp_output_272_0 = ( temp_output_271_0 + half3(0,0,0) );
			float TranslationSpeed329 = _TranslationSpeed;
			float2 temp_cast_12 = (TranslationSpeed329).xx;
			float2 uv0_DetailMap = i.uv_texcoord * _DetailMap_ST.xy + _DetailMap_ST.zw;
