...
public class ParseDialogue : Editor
{
	...
	static void ParseDia(SheetParser ps)
	{
		...
		for (int i = ps.cardinality - 1; i >= 1; --i)
		{
			if (!npcDatas.ContainsKey(ps.GetAttribute(i, NPCNAME)))
			{
				...
				Debug.Log(ps.GetAttribute(i, NPCNAME) + "데이터추가됨 " + cur.name);
			...
			else
			{
				...
				if (npcDatas[ps.GetAttribute(i, NPCNAME)].ContainsKey(convSection))
				{
					...
					Debug.Log(ps.GetAttribute(i, NPCNAME) + "에다가 데이터추가됨 " + cur.name);
				...
				else
				{
					...
					npcDatas[ps.GetAttribute(i, NPCNAME)].Add(convSection, dia);
				}
			}
		}
